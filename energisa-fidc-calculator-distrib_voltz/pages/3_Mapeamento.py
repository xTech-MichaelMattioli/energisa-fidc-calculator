        ...
        with st.spinner("🔄 Aplicando mapeamentos e padronizando dados..."):
            try:
                ...
                if dataframes_padronizados:
                    ...
                    st.info("� Padronizando estrutura dos dados...")
                    ...
                    if eh_voltz:
                        ...
                        st.info("📊 **Distribuidora padrão** - Mantendo todos os registros (duplicatas não removidas)")
                        
                        # Para outras distribuidoras também não remover duplicatas
                        # Apenas fazer limpeza de colunas auxiliares se existirem
                        
                        # Criar colunas auxiliares apenas para limpeza posterior (não para comparação)
                        colunas_auxiliares_criadas = []
                        ...
                        if 'nome_cliente' in df_final_padronizado.columns:
                            ...
                            colunas_auxiliares_criadas.append('nome_cliente_limpo')
                        ...
                        if 'data_vencimento' in df_final_padronizado.columns:
                            ...
                            colunas_auxiliares_criadas.append('data_vencimento_limpa')
                        
                        # Remover as colunas auxiliares criadas (não precisamos delas)
                        if colunas_auxiliares_criadas:
                            df_final_padronizado = df_final_padronizado.drop(
                                columns=colunas_auxiliares_criadas
                            )
                        
                        st.success("✅ **Padrão**: Todos os registros mantidos (duplicatas não removidas)")
                    
                    # Salvar resultado
                    st.session_state.df_padronizado = df_final_padronizado
                    st.session_state.dataframes_individuais = dataframes_padronizados
                    
                    st.success(f"🎯 **Mapeamento concluído!** {len(df_final_padronizado):,} registros finais de {len(dataframes_padronizados)} arquivo(s)")
                    
                    # Mostrar informação sobre manutenção de todos os registros
                    if registros_antes == len(df_final_padronizado):
                        st.info(f"📊 Todos os {registros_antes:,} registros foram mantidos (nenhuma duplicata removida)")
                    else:
                        # Isso não deve acontecer agora, mas mantemos como fallback
                        st.info(f"📊 Registros processados: {registros_antes:,} → {len(df_final_padronizado):,}")
